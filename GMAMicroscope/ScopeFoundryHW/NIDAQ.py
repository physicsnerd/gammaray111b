...
class NI_TaskWrap(object):
    ...
    def error(self, err ):
            ...
            logger.error('Error calling "{}": {}'.format( err.fname, err.message ))
    ...
    def get_devices(self):
        ...
        #mx.DAQmxGetDevAIPhysicalChans( AIdev, chanList, buffSize ) ########################################Maybe this line?#############

class NI_DacTask(NI_TaskWrap): #NEED TO MODIFY THIS TASK ##
    '''
    ...
    def set_channel(self, channel ):
        ...
        try:                
            ...
            #self.task.CreateAOVoltageChan(self._channel, '', -10.0, +10.0, mx.DAQmx_Val_Volts, '')  ################# OG ##################################
            
            # self.task.CreateAIVoltageChan(self._channel,"Dev1/0","",
            #                        mx.DAQmx_Val_Cfg_Default,
            #                        float64(-10.0),float64(10.0),
            #                        DAQmx_Val_Volts,None)
            self.task.CreateAIVoltageChan(self._channel, '',mx.DAQmx_Val_Cfg_Default,-10.0,+10.0, mx.DAQmx_Val_Volts, '')
            #self.task.CreateAIVoltageChan(self._channel, '', -10.0, 10.0, mx.DAQmx_Val_Volts, '') ####OG but not working either############
            #self.task.ai_channels.add_ai_volrage_chan("Dev1/0")
    ...
    def read(self):
        ...
        #self.task.ReadAnalogF64(timeout=10.0, value = mx.byref(value),reserved=None)
        #self.task.ReadAnalogF64(timeout=10.0, value = mx.byref(value),reserved=None)
        return value.value
    ...
    def load_buffer(self, data, auto = False, timeout=0 ):
        ...
        try:
            ...
            self.task.ReadAnalogF64(dac_samples, #int 32 numSampsPerChan 1 works
                                    1.0, #Float64 timeout 0 indicates to try once to read the requested samples 1.0 works
                                    mx.DAQmx_Val_GroupByChannel, #bool32fillMode
                                    data,  #float64 readArray[]
                                    100, #uInt32 arraySizeInSamps QUESTION MARK??
                                    mx.byref(writeCount), #int32 *sampsPerChanRead
                                    None) # bool32 *reserved
        except mx.DAQError as err:
            #print("sample load count {} transfer count {}".format( self._sample_count, writeCount.value )) #previosly commented out
            self.error(err)
    ...
    def set(self, data):
        ...
        try:
            ...
            #                       data, mx.byref(writeCount), None)
            self.task.ReadAnalogF64(1, #int 32 numSampsPerChan
                                    1.0, #Float64 timeout 0 indicates to try once to read the requested samples 1.0 works
                                    mx.DAQmx_Val_GroupByChannel, #bool32fillMode
                                    data,  #float64 readArray[]
                                    10, #uInt32 arraySizeInSamps QUESTION MARK??
                                    mx.byref(writeCount), #int32 *sampsPerChanRead
                                    None) # bool32 *reserved
...
class NIDAQHardware(HardwareComponent):
    ...
    def setup(self):
        self.settings.New('dac_val', dtype=float, ro=False,  unit='V', initial=0.0)
        #self.settings.New('channel', dtype=str, initial='/Dev1/ao0') #################### ORIGINAL ###################################################################
        #AO_00, Ao_00, AO_0, 
        self.settings.New('channel', dtype=str, initial='/Dev1/0')  ######New one######### /Dev1/0 or Dev1/0
    def connect(self):
        S = self.settings
        
        # Open connection to hardware
        self.dac_task = NI_DacTask(channel=S['channel'], name=self.name)
        
        self.dac_task.set_single() #commented out just now
        self.dac_task.start()
        
        #TODO disable channel and terminal_config 
        
        #connect settings to hardware
        self.settings.dac_val.connect_to_hardware(read_func=self.dac_task.read) #changed from write_func OG
        #self.dac_task.set(self.settings["dac_val"]) #This one works
        
    def disconnect(self):
        self.settings.disconnect_all_from_hardware()

        #enable channel and terminal_config 
